...
{	
	...
	cs_config.output_level = GPIO_LEVLE_LOW;
	gpio_pin_config(GPIO1, 20, &cs_config);
	
	/* 2、初始化SPI */
	imx6ull_spi_init(ECSPI3);	
...
void icm20608_write_reg(unsigned char reg, unsigned char value)
{
	...
	spich0_read_write_byte(ECSPI3, reg); 	/* 发送寄存器地址		*/ 
	spich0_read_write_byte(ECSPI3, value);	/* 发送要写入的值			*/
...
unsigned char icm20608_read_reg(unsigned char reg)
{
	...
	reg |= 0x80; 	
   	...
   	ICM20608_CSN(0);               					/* 使能SPI传输	 		*/
  	spich0_read_write_byte(ECSPI3, reg);     		/* 发送寄存器地址  		*/ 
  	reg_val = spich0_read_write_byte(ECSPI3, 0XFF);	/* 读取寄存器的值 			*/
 	ICM20608_CSN(1);                				/* 禁止SPI传输 			*/
...
{  
	...
	reg |= 0x80; 
 	...
 	{
		buf[i] = spich0_read_write_byte(ECSPI3, 0XFF);	
	}
 	ICM20608_CSN(1);                			/* 禁止SPI传输 			*/
}
