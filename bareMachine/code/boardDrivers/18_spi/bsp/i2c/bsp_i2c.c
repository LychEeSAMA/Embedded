 ...
 * @return 				: 0 正常 其他值 出错
 */
static unsigned char i2c_master_repeated_start(I2C_Type *base, unsigned char address,  enum i2c_direction direction)
{
	/* I2C忙并且工作在从模式,跳出 */
 ...
 * @return 			: 无
 */
static void i2c_master_write(I2C_Type *base, const unsigned char *buf, unsigned int size)
{
	/* 等待传输完成 */
	while(!(base->I2SR & (1 << 7))); 
...
static void i2c_master_read(I2C_Type *base, unsigned char *buf, unsigned int size)
{
	...
	dummy = base->I2DR; /* 假读 当完成start和reStart后 读后面的数据需要先接收ACK应答 所以需要先读一次把ACK去掉 不然后面数据会错位 */
 ...
 * @return 			: 无
 */
void imx6ull_i2c_init(I2C_Type *base)
{
	/* 1、配置I2C */
