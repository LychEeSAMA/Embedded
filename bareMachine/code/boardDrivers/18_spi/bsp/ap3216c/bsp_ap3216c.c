 ...
 * @return 		: 操作结果
 */
static unsigned char ap3216c_write_one_byte(unsigned char addr,unsigned char reg, unsigned char data)
{
 ...
 * @return 		: 读取到的数据。
 */
static unsigned char ap3216c_read_one_byte(unsigned char addr,unsigned char reg)
{
	unsigned char val=0;
...
unsigned char ap3216c_init(void)
{
	...
	/* Inputs of I2Cn_SCL and I2Cn_SDA also need to be manually enabled by setting the
	   ...
	   remark:对于GPIO SION配置为1时 即使此引脚做输出 此时也通过psr寄存器获取其值
	*/
