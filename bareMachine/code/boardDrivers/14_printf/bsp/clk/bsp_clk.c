...
void imx6ull_clk_init(void)
{
	...
	/* 
		修改ARM内核时钟频率流程如下：
		首先ARM_CLK来源于pll1_sw_clk，pll1_sw_clk提供的时钟通过CACRR[ARM_PODF](Arm Clock Root Register)进行分频，
		最终得到ARM_CLK供内核使用。
		而pll1_sw_clk有两个来源一个是pll1_main_clk，另一个是step_clk,其中step_clk的来源有两个，一个为24MHZ的OSC，另一个不关心用不到

		我们要修改pll1_main_clk的频率，需要先将pll1_sw_clk的提供者修改为step_clk，然后再对pll1_main_clk进行修改
		这里我们首先需要修改step_clk的时钟源为24MHZ的OSC，然后将pll1_sw_clk的时钟源修改为step_clk
		然后我们对pll1_main_clk的频率进行修改，PLL ranges from 650 MHz to 1.3 GHz
		计算公式为PLL output frequency = Fref * DIV_SEL/2

		设置pll1_main_clk为792MHz
	...
	*/
	 ...
	 *              		得出：div_select=    66 
	 */
	CCM_ANALOG->PLL_ARM = (1 << 13) | ((58 << 0) & 0X7F); 	/* 配置pll1_main_clk=792MHz */
...
* 等待握手完成 */
	...
	/* 7、设置uart根时钟为pll3_80M */
	CCM->CSCDR1 &= ~(1 << 6);	/* UART_CLK_ROOT=80MHZ */
	CCM->CSCDR1 &= ~0x3F;	    /* 即1分频 */
}
