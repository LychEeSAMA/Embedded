...
static void lcd_gpio_init(void)
{
	...
	/* GPIO初始化 */
	gpio_config.direction = GPIO_OUTPUT;		/* 输出 */
	gpio_config.output_level = GPIO_LEVLE_HIGH; /* 默认关闭背光 */
	gpio_pin_config(GPIO1, 8, &gpio_config);	/* 背光默认打开 */
 ...
 * @return 		: 屏幕ID
 */
static unsigned short lcd_read_panel_id(void)
{
	unsigned char idx = 0;
	...
	/* 打开模拟开关 */
	GPIO_Config idio_config;
	idio_config.direction = GPIO_OUTPUT;
	idio_config.output_level = GPIO_LEVLE_HIGH;
	gpio_pin_config(GPIO3, 3, &idio_config);
	...
	gpio_pin_config(GPIO3, 12, &idio_config);
	gpio_pin_config(GPIO3, 20, &idio_config);
	gpio_pin_config(GPIO3, 28, &idio_config);
 ...
 * @return 		: 无
 */
static void lcd_stop_reset(void)
{
 ...
 * @return 		: 无
 */
void imx6ull_lcd_init(void)
{
	unsigned short lcdid = 0;

	lcdid = lcd_read_panel_id(); /* 读取屏幕ID值 		*/
	printf("LCD ID=%#X\r\n", lcdid);
